...
($project)=$ARGV[0]=~/^([^\/ ]*)[ \/]?/;
$mailfile="$CVSROOT/CVSROOT/mailto";
$MAIL_TO='';
open(M,$mailfile) || die "can't open $mailfile: $!";
while(<M>) {
    chomp;
    ($p,$list)=split /:/;
    if($p eq $project || $p eq '*') {
	$MAIL_TO.=' '.$list;
    }
}
close M;
...
sub mail_notification
{
    ...
    unless ($MAIL_TO)
    {
	warn "MAIL_TO undefined";
	$MAIL_TO = 'tds@aldigital.co.uk';
    }
    
    my $what = "mail -s \"cvs commit: $ARGV[0]\" $MAIL_TO";
    print "Mailing the commit message to $MAIL_TO: $what\n";

    (open(MAIL, "|$what")) or warn "Mail failed: $!";
    print(MAIL join("\n", @text));
    (close(MAIL)) or warn "Mail failed: $!";
}
