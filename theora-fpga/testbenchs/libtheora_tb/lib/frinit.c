...
static void CalcPixelIndexTable( PB_INSTANCE *pbi){
...
#ifdef RECON_PIXEL_INDEX
  fp = fopen(IN_RPI_FILE, "a");
  fprintf(fp, "%d\n", pbi->HFragments);
  fprintf(fp, "%d\n", pbi->YPlaneFragments);
  fprintf(fp, "%d\n", pbi->YStride);
  fprintf(fp, "%d\n", pbi->UVPlaneFragments);
  ...
  fprintf(fp, "%d\n", pbi->ReconVDataOffset);
  for (i=0; i < pbi->YPlaneFragments + 2*pbi->UVPlaneFragments; i++) {
    fprintf(fp, "%d\n", i);
  }
  fclose(fp);

  fp = fopen(OUT_RPI_FILE, "a");
  for (i=0; i < pbi->YPlaneFragments + 2*pbi->UVPlaneFragments; i++) {
...
void InitFragmentInfo(PB_INSTANCE * pbi){
  ...
  pbi->DispFragLen = pbi->UnitFragments * sizeof(*pbi->display_fragments);
  ...
  pbi->ReconPixITLen = pbi->UnitFragments * sizeof(*pbi->recon_pixel_index_table);
...
void InitFrameInfo(PB_INSTANCE * pbi, unsigned int FrameSize){
  ...
  pbi->LastFraRecLen = FrameSize*sizeof(*pbi->LastFrameRecon);
