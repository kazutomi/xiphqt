...
static void SetupBoundingValueArray_Generic(PB_INSTANCE *pbi,
  ...
  /* Fill(512*sizeof(*pbi->FiltBoundingValue)) bytes of pbi->FiltBoundingValue with 0 */
...
void CopyFilterTables(PB_INSTANCE *pbi, codec_setup_info *ci){
  ...
  FILE *fp;
  int aux, aux2, meuInt;
  fp = fopen(IN_LF_FILE, "a");
#endif
  memcpy(pbi->LoopFilterLimits, ci->LoopFilterLimitValues, Q_TABLE_SIZE);

#ifdef LOOP_FILTER
  /*Matriz LoopFilterLimits*/
  for (meuInt = 0; meuInt < 64; meuInt=meuInt+4) {
    aux = 0;
    for (aux2 = 0; aux2 < 4; aux2++) {
      aux = (aux << 8);
      aux = aux + ((unsigned int)pbi->LoopFilterLimits[meuInt + aux2]);
    }
    fprintf(fp, "%d\n", aux);
  }
  fclose(fp);
...
static int TesteBench(PB_INSTANCE *pbi) {
  return !(pbi->Count % MAXCOUNT) && (pbi->Count >= 31900/*26400*/);
}
...
static void ExpandBlock ( PB_INSTANCE *pbi, ogg_int32_t FragmentNumber ){
  ...
  FILE *fp, *fp2;
  static int vezes = 0;
  int i, aux, meuInt, aux2;
  pbi->Count = pbi->Count + 1;
  if (TesteBench(pbi) && (vezes != VEZES_EB)) {
    fp = fopen(IN_EB_FILE, "a");
    ...
    for (i = 0; i < 64; i++) {
      fprintf(fp, "%d\n", pbi->QFragData[FragmentNumber][i]);
    }

    fprintf(fp, "%d\n", pbi->FragCodingMethod[FragmentNumber]);
    fprintf(fp, "%d\n", pbi->FragCoefEOB[FragmentNumber]);
    fprintf(fp, "%d\n", pbi->FragMVect[FragmentNumber].x);
    ...
    fprintf(fp, "%d\n", (pbi->ReconYPlaneSize + 2 * pbi->ReconUVPlaneSize)*2);
  ...
  if (TesteBench(pbi) && (vezes != VEZES_EB)) {
    ...
    for (meuInt = 0; meuInt < pbi->LastFraRecLen; meuInt=meuInt+4) {
      ...
      for (aux2 = 0; aux2 < 4; aux2++) {
	aux = (aux << 8);
	aux = aux + ((unsigned int)pbi->GoldenFrame[meuInt + aux2]);
    ...
    for (meuInt = 0; meuInt < pbi->LastFraRecLen; meuInt=meuInt+4) {
      ...
      for (aux2 = 0; aux2 < 4; aux2++) {
	aux = (aux << 8);
	aux = aux + ((unsigned int)pbi->LastFrameRecon[meuInt + aux2]);
      }
      fprintf(fp2, "%d\n", aux);
    }

    /*Matriz ThiFrameRecon*/
    fprintf(fp2, "%d\n", pbi->LastFraRecLen);
    for (meuInt = 0; meuInt < pbi->LastFraRecLen; meuInt=meuInt+4) {
      aux = 0;
      for (aux2 = 0; aux2 < 4; aux2++) {
	aux = (aux << 8);
	aux = aux + ((unsigned int)pbi->ThisFrameRecon[meuInt + aux2]);
      }
      fprintf(fp2, "%d\n", aux);
    }
...
void LoopFilter(PB_INSTANCE *pbi){
  ...
  if (vezes != VEZES_LF) {
    fp = fopen(IN_LF_FILE, "a");
    fp2 = fopen(IN2_LF_FILE, "a");

    /*Matriz display_fragments*/

    aux = 0;
    b = pbi->UnitFragments / 32;

    for (meuInt = 0; meuInt < b; meuInt++) {
      for (aux2 = 0; aux2 < 32; aux2++)
	aux = (aux << 1) + pbi->display_fragments[meuInt*32 + aux2];
      fprintf(fp, "%d\n", aux);
    }
    aux = 0;
    for (meuInt = 0; meuInt < (pbi->UnitFragments % 32); meuInt++) {
      aux = (aux << 1) + pbi->display_fragments[b*32 + meuInt];
    }
    if (pbi->UnitFragments % 32) {
      aux = (aux << (32 - (pbi->UnitFragments % 32)));
      fprintf(fp, "%d\n", aux);
    }
  ...
  if (vezes != VEZES_LF) {
    ...
    fp2 = fopen(OUT_LF_FILE, "a");
    for (count = 0; count < pbi->LastFraRecLen; count=count+4) {
      aux = 0;
      for (aux2 = 0; aux2 < 4; aux2++) {
	aux = (aux << 8);
	aux = aux + ((unsigned int)pbi->LastFrameRecon[count + aux2]);
      }
      fprintf(fp2, "%d\n", aux);
    }
...
void ReconRefFrames (PB_INSTANCE *pbi){
  ...
  if (vezes2 < VEZES_CR) {
    ...
    for (c = 0; c < b; c++) {
      for (d = 0; d < 32; d++)
	a = (a << 1) + pbi->display_fragments[c*32 + d];
      fprintf(fp3, "%d\n", a);
    }
    a = 0;
    for (c = 0; c < (pbi->UnitFragments % 32); c++) {
      a = (a << 1) + pbi->display_fragments[b*32 + c];
    }
    if (pbi->UnitFragments % 32) {
      a = (a << (32 - (pbi->UnitFragments % 32)));
      fprintf(fp3, "%d\n", a);
    }
    fprintf(fp3, "%d\n", 287232);
    fprintf(fp3, "%d\n", 143616);
    ...
    fp3 = fopen(IN2_CR_FILE, "a");
    /*Matriz GoldenFrame*/
    fprintf(fp3, "%d\n", pbi->LastFraRecLen);
    for (a = 0; a < pbi->LastFraRecLen; a=a+4) {
      c = 0;
      for (b = 0; b < 4; b++) {
	c = (c << 8);
	c = c + ((unsigned int)pbi->GoldenFrame[a + b]);
    ...
    for (a = 0; a < pbi->LastFraRecLen; a=a+4) {
      ...
      for (b = 0; b < 4; b++) {
	c = (c << 8);
	c = c + ((unsigned int)pbi->LastFrameRecon[a + b]);
      }
      fprintf(fp3, "%d\n", c);
    }
    ...
    for (a = 0; a < pbi->LastFraRecLen; a=a+4) {
      ...
      for (b = 0; b < 4; b++) {
	c = (c << 8);
	c = c + ((unsigned int)pbi->ThisFrameRecon[a + b]);
      }
      fprintf(fp3, "%d\n", c);
    }
    fclose(fp3);
