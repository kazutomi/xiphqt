...
*/
  ...
  for (bytes = 0; bytes < 4096 && n_vector < MAX_N_VECTOR ; bytes++, n_vector++)
    buffer[bytes] = vector_of_input[n_vector];
  ogg_sync_wrote(oy,bytes);
  return(bytes);
...
/*
  if(!raw)
    printf(outfile, "YUV4MPEG2 W%d H%d F%d:%d I%c A%d:%d\n",
