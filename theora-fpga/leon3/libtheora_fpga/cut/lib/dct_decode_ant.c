...
//  else
  ...
  flag_first_time = 0;
    ...
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->ReconYPlaneSize + 2 * pbi->ReconUVPlaneSize;

    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->info.height - 8;

    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->info.width;
        
    //fprintf(fp_rr, "%d\n", pbi->HFragments);
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->HFragments;     
    
    //fprintf(fp_rr, "%d\n", pbi->YPlaneFragments);
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->YPlaneFragments;
    ...
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->UVPlaneFragments;
    
    //fprintf(fp_rr, "%d\n", pbi->UVStride);
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->UVStride;
    
    //fprintf(fp_rr, "%d\n", pbi->VFragments);
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->VFragments;
    
    //fprintf(fp_rr, "%d\n", pbi->ReconYDataOffset);
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->ReconYDataOffset;
    
    //fprintf(fp_rr, "%d\n", pbi->ReconUDataOffset);
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    dt.data = pbi->ReconUDataOffset;
    
    //fprintf(fp_rr, "%d\n", pbi->ReconVDataOffset);
    do ioctl(pf, 0, &dt); while (dt.leu == 0); 
    dt.data = pbi->ReconVDataOffset;
    
    //fprintf(fp_rr, "%d\n", pbi->UnitFragments);
    do ioctl(pf, 0, &dt); while (dt.leu == 0); 
    dt.data = pbi->UnitFragments;
    do ioctl(pf, 0, &dt); while (dt.leu == 0);
    /* QThreshTable */
    for (int1 = 0; int1 < 64; int1++) {
      //fprintf(fp_rr, "%d\n", pbi->QThreshTable[int1]);
      
      dt.data = pbi->QThreshTable[int1];
      do ioctl(pf, 0, &dt); while (dt.leu == 0); 
    }
      ...
      for ( n = 0 ; n < FragsAcross ; n++, i++){
        ...
        if( pbi->display_fragments[i] || (GetFrameType(pbi) == KEY_FRAME) ){
          ...
          if (flag_first_iteration) {
             dt.data = pbi->CodedBlockIndex;
	     do ioctl(pf, 0, &dt); while (dt.leu == 0);

	      for (int1 = 0; int1 < 64; int1++) {
                dt.data = pbi->dequant_Y_coeffs[int1];
                do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	      }
	      for (int1 = 0; int1 < 64; int1++) {
                dt.data = pbi->dequant_U_coeffs[int1];
                do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	      }
	      for (int1 = 0; int1 < 64; int1++) {
                dt.data = pbi->dequant_V_coeffs[int1];
                do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	      }
	      for (int1 = 0; int1 < 64; int1++) {
                dt.data = pbi->dequant_InterY_coeffs[int1];
                do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	      }
	      for (int1 = 0; int1 < 64; int1++) {
                dt.data = pbi->dequant_InterU_coeffs[int1];
                do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	      }
	      for (int1 = 0; int1 < 64; int1++) {
                dt.data = pbi->dequant_InterV_coeffs[int1];
                do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	      }
              dt.data = pbi->FrameType;
              do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	      flag_first_iteration = 0;
	    }
	    for (int1 = 0; int1 < 64; int1++) {
              dt.data = pbi->QFragData[i][int1];
              do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	    }
            
	    dt.data = pbi->FragCodingMethod[i];
            do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	    
            dt.data = pbi->FragCoefEOB[i];
	    do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	    
            dt.data = pbi->FragMVect[i].x;
	    do ioctl(pf, 0, &dt); while (dt.leu == 0); 
	    
            dt.data = pbi->FragMVect[i].y;
	    do ioctl(pf, 0, &dt); while (dt.leu == 0); 
  ...
  //printf("antes copy recon !!! \n");
    ...
    dt.data = pbi->ThisFrameQualityValue;
    do {ioctl(pf, 0, &dt); /*printf("b");*/ }while (dt.leu == 0);
    //fclose(fp_rr);
...
// //  }
	...
	/*
        ...
        dt.escreveu = 0;
	if (ioctl(pf, 1, &dt) < 0) {
          //fprintf(fsaida,"ERROR: ioctl(0,&a) failed\n");
	  printf("ERROR: ioctl(0,&a) failed\n");
       // return 1;
        }
	//if (dt.escreveu) fprintf(fsaida,"--> %d\n", dt.data);
      } while (dt.escreveu == 0);
  */

  /*
  //fsaida = fopen("out.txt", "w");
