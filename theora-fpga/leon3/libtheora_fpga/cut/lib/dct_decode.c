...
//  else
  ...
  if (is_first_time(-1)) {
    ...
    write_theora_apb(0);
    write_theora_apb(pbi->ReconYPlaneSize + 2 * pbi->ReconUVPlaneSize);
    write_theora_apb(pbi->info.height);
    write_theora_apb(pbi->info.width);
    write_theora_apb(pbi->HFragments);
    write_theora_apb(pbi->YPlaneFragments);
    write_theora_apb(pbi->YStride);
    write_theora_apb(pbi->UVPlaneFragments);
    write_theora_apb(pbi->UVStride);
    write_theora_apb(pbi->VFragments);
    write_theora_apb(pbi->ReconYDataOffset);
    write_theora_apb(pbi->ReconUDataOffset);
    write_theora_apb(pbi->ReconVDataOffset);
    write_theora_apb(pbi->UnitFragments);
    
    /* QThreshTable */
    for (int1 = 0; int1 < 64; int1++) {
      write_theora_apb(pbi->QThreshTable[int1]);
    }
  ...
  for ( j = 0; j < 3 ; j++) {
	...
	if (flag_CodedBlockIndex) {
            write_theora_apb(pbi->CodedBlockIndex);
	    ...
	    //  write_theora_apb(pbi->CodedBlockIndex);
	      for (int1 = 0; int1 < 64; int1++)
	        write_theora_apb(pbi->dequant_Y_coeffs[int1]);
	      for (int1 = 0; int1 < 64; int1++)
  	        write_theora_apb(pbi->dequant_U_coeffs[int1]);
	     for (int1 = 0; int1 < 64; int1++)
	        write_theora_apb(pbi->dequant_V_coeffs[int1]);
	      for (int1 = 0; int1 < 64; int1++)
	        write_theora_apb(pbi->dequant_InterY_coeffs[int1]);
	      for (int1 = 0; int1 < 64; int1++)
	        write_theora_apb(pbi->dequant_InterU_coeffs[int1]);
	      for (int1 = 0; int1 < 64; int1++)
	        write_theora_apb(pbi->dequant_InterV_coeffs[int1]);
	      write_theora_apb(pbi->FrameType);
	      flag_first_iteration = 0;
	  }
	    
	    for (int1 = 0; int1 < 64; int1++)
	       write_theora_apb(pbi->QFragData[i][int1]);
	    write_theora_apb(pbi->FragCodingMethod[i]);
	    write_theora_apb(pbi->FragCoefEOB[i]);
	    write_theora_apb(pbi->FragMVect[i].x);
	    write_theora_apb(pbi->FragMVect[i].y);
	    write_theora_apb(i);
    ...
    write_theora_apb(pbi->ThisFrameQualityValue);
