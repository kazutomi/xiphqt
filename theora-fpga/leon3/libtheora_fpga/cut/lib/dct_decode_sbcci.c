 ...
 ********************************************************************
  ...
  function:
  last mod: $Id: dct_decode.c 11442 2006-05-27 17:28:08Z giles $
...
void ReconRefFrames (PB_INSTANCE *pbi){
  ...
  //static int flag_first_time = 1;
  int flag_CodedBlockIndex = 1;
...
theora_regs.data_received = (volatile int  *)0x8000080C;
  if (!flag_theora) {
    //pf = open("/dev/theora",O_RDONLY|O_WRONLY|O_TRUNC|O_CREAT);
    flag_theora = 1;
  }
...
//  else
  ...
  SetupLoopFilter(pbi);
    ...
    //printf("reconrefframes!!!!\n");   
    
  if (is_first_time(-1)) {
    is_first_time(0);

    write_theoradriver(pf, 0);
    write_theoradriver(pf, pbi->ReconYPlaneSize + 2 * pbi->ReconUVPlaneSize);
    write_theoradriver(pf, pbi->info.height);
    write_theoradriver(pf, pbi->info.width);
    write_theoradriver(pf, pbi->HFragments);
    write_theoradriver(pf, pbi->YPlaneFragments);
    write_theoradriver(pf, pbi->YStride);
    write_theoradriver(pf, pbi->UVPlaneFragments);
    write_theoradriver(pf, pbi->UVStride);
    write_theoradriver(pf, pbi->VFragments);
    write_theoradriver(pf, pbi->ReconYDataOffset);
    write_theoradriver(pf, pbi->ReconUDataOffset);
    write_theoradriver(pf, pbi->ReconVDataOffset);
    write_theoradriver(pf, pbi->UnitFragments);
    
    /* QThreshTable */
    for (int1 = 0; int1 < 64; int1++) {
      write_theoradriver(pf, pbi->QThreshTable[int1]);
    }
  ...
  for ( j = 0; j < 3 ; j++) {
    ...
    for ( m = 0 ; m < FragsDown ; m++) {
      for ( n = 0 ; n < FragsAcross ; n++, i++){
        ...
        if (flag_CodedBlockIndex) {
            write_theoradriver(pf, pbi->CodedBlockIndex);
            flag_CodedBlockIndex = 0;
        }      
        ...
        if( pbi->display_fragments[i] || (GetFrameType(pbi) == KEY_FRAME) ){
	      ...
	      for (int1 = 0; int1 < 64; int1++)
	        write_theoradriver(pf, pbi->dequant_Y_coeffs[int1]);
	      for (int1 = 0; int1 < 64; int1++)
  	        write_theoradriver(pf, pbi->dequant_U_coeffs[int1]);
	     for (int1 = 0; int1 < 64; int1++)
	        write_theoradriver(pf, pbi->dequant_V_coeffs[int1]);
	      for (int1 = 0; int1 < 64; int1++)
	        write_theoradriver(pf, pbi->dequant_InterY_coeffs[int1]);
	      for (int1 = 0; int1 < 64; int1++)
	        write_theoradriver(pf, pbi->dequant_InterU_coeffs[int1]);
	      for (int1 = 0; int1 < 64; int1++)
	        write_theoradriver(pf, pbi->dequant_InterV_coeffs[int1]);
	      write_theoradriver(pf, pbi->FrameType);
	  }
	    
	    for (int1 = 0; int1 < 64; int1++)
	       write_theoradriver(pf, pbi->QFragData[i][int1]);
	    write_theoradriver(pf, pbi->FragCodingMethod[i]);
	    write_theoradriver(pf, pbi->FragCoefEOB[i]);
	    write_theoradriver(pf, pbi->FragMVect[i].x);
	    write_theoradriver(pf, pbi->FragMVect[i].y);
	    write_theoradriver(pf, i);
	...
	int1 = (int1 << 1) + pbi->display_fragments[int3*32 + int4];
        write_theoradriver(pf, int1);
    }
    
    for (int1 = 0, int3 = 0; int3 < (pbi->UnitFragments % 32); int3++)
      int1 = (int1 << 1) + pbi->display_fragments[int2*32 + int3];

    if (pbi->UnitFragments % 32)
      write_theoradriver(pf, (int1 << (32 - (pbi->UnitFragments % 32))));

    write_theoradriver(pf, pbi->ThisFrameQualityValue);
...
// //  }
	...
	/*
        ...
        dt.escreveu = 0;
	if (ioctl(pf, 1, &dt) < 0) {
          //fprintf(fout,"ERROR: ioctl(0,&a) failed\n");
	  printf("ERROR: ioctl(0,&a) failed\n");
       // return 1;
        }
	//if (dt.escreveu) fprintf(fout,"--> %d\n", dt.data);
      } while (dt.escreveu == 0);
  */
  ...
  if (vezes_rr < VEZES_RR && flag_rr) {
	...
	//if (dt.escreveu) fprintf(fout,"--> %d\n", dt.data);
      ...
      //printf("saida [%d] = soft %d : hard %d\n", contout++, int3, dt.data);
